...
Written by Jeremy Selva [{{< fa solid link >}}](https://jeremy-selva.netlify.app/) [{{< fa brands github >}}](https://github.com/JauntyJJS) [{{< fa brands linkedin >}}](https://www.linkedin.com/in/jeremy-selva-085b9112a) [{{< fa brands twitter >}}](https://twitter.com/jauntyjjs)

# Introduction

Here is a report showing how to create injection sequence plot and dilution plot using [`plotly`](https://plotly.com/r/) for each Multiple Reaction Monitoring (MRM) transition in Mass Spectrometry.
...
The injection sequence plot is a scatter plot with the peak area (or concentration) on the y-axis and the injection sequence order (or data acquisition time) on the x axis. This is to better understand the variation of peak area in the quality control (QC) sample types over time. A [raincloud plot](https://wellcomeopenresearch.org/articles/4-63) [@Allen2021] is placed on the right of the scatter plot to show the distribution of each (QC) sample type as well as to highlight any potential outliers.
...
area_data <- readr::read_csv("https://raw.github.com/JauntyJJS/Trelliscopejs_In_Quarto_Example/main/data/RP-UHPLC_MS_Area.csv")
```
...
```{r read transition name annotation}
transition_name_annot <- readr::read_csv("https://raw.github.com/JauntyJJS/Trelliscopejs_In_Quarto_Example/main/data/RP-UHPLC_MS_Transition_Name_Annot.csv")
...
| Lipid_Name                    | Converted transition name to suit lipid nomenclature set by [Liebisch et. al. 2020](https://doi.org/10.1021/acs.jafc.0c07175) [@Liebisch2020]. |
| For_Rgoslin                   | Input transition for the R package [rgoslin](https://github.com/lifs-tools/rgoslin) [@GOSLIN], [@GOSLIN2] to annotate.                         |
...
```{r preview transition name annotation}
#| code-fold: true
...
```{r read sample annotation}
sample_annot <- readr::read_csv("https://raw.github.com/JauntyJJS/Trelliscopejs_In_Quarto_Example/main/data/RP-UHPLC_MS_Sample_Annot.csv")
```
...
+-----------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| QC_Sample_Type        | Quality control category of the sample. More information can be found in [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                                                        |
|                       |                                                                                                                                                                                                                                                         |
...
|                       |                                                                                                                                                                                                                                                         |
|                       | -   NIST stands for SRM1950 NIST plasma sample from [Simón-Manso et. al. 2013](https://doi.org/10.1021/ac402503m) [@Simón-Manso2013] . It is a standard reference material provides a method to allow quality assessment across different laboratories. |
+-----------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
```{r read dilution annotation}
dilution_annot <- readr::read_csv("https://raw.github.com/JauntyJJS/Trelliscopejs_In_Quarto_Example/main/data/RP-UHPLC_MS_Dilution_Annot.csv")
...
```{r merge and nest sample annotation data}
...
nested_sample_data <- sample_annot |> 
  ...
  tidyr::nest() |>
  dplyr::rename(sample_data = .data[["data"]]) |> 
  dplyr::ungroup()
...
```{r preview nested sample annotation}
#| code-fold: true
...
```{r merge and nest sample dilution data}
...
nested_dilution_data <- dilution_annot |> 
  ...
  dplyr::group_by(.data[["Transition_Name"]]) |>
  tidyr::nest() |>
  dplyr::rename(dilution_data = .data[["data"]]) |> 
  dplyr::ungroup()
```
...
```{r preview nested dilution annotation}
#| code-fold: true
...
We now add the annotations for each transitions from `transition_name_annot` and [rgoslin](https://github.com/lifs-tools/rgoslin).

## Add `transition_name_annot` annotations

We first join the nested_data with our transition annotations.

```{r add transition_name_annot annotation}
...
The R package [rgoslin](https://github.com/lifs-tools/rgoslin) is useful in getting additional information of lipid names such as the lipid classes. A column called `For_Rgoslin` was created to convert each transition name that can be parsed by [rgoslin](https://github.com/lifs-tools/rgoslin). Unfortunately only [Coenzyme Q10](https://www.lipidmaps.org/databases/lmsd/LMPR02010001?LMID=LMPR02010001) is unable to be parsed by [rgoslin](https://github.com/lifs-tools/rgoslin). We thus add the relevant information manually.

Below is an code example (not part of the pipeline) on how [rgoslin](https://github.com/lifs-tools/rgoslin) can be used

```{r rgoslin demo}

rgoslin_example <- annotated_data |> 
  dplyr::select(.data[["For_Rgoslin"]]) |> 
...
For the workflow, we do not need all the columns that [rgoslin](https://github.com/lifs-tools/rgoslin) provide.

The following columns are kept instead.

| Column Name               | Column Description                                                                                                                                          |
|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Lipid.Maps.Category       | [LIPID MAPS'](https://www.lipidmaps.org/databases/lmsd/browse) Structure Database [@Subramaniam2006] Lipid Category which the transition name belongs to.   |
| Lipid.Maps.Main.Class     | [LIPID MAPS'](https://www.lipidmaps.org/databases/lmsd/browse) Structure Database [@Subramaniam2006] Lipid Main Class which the transition name belongs to. |
...
```{r add rgoslin annotation, warning=FALSE, message=FALSE}
...
```{r calculate dilution statistics}
...
summary_data <- annotated_data |> 
  ...
  tidyr::unnest(.data$dilution_summary) |> 
  dplyr::mutate(
...
| Column Name              | Column Description                                                                                                                                                                                     |
|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| bqc_mean                 | Mean signal from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                            |
| bqc_median               | Median signal from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                          |
| bqc_sd                   | Standard deviation of signal from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                           |
| bqc_mad                  | Median absolute deviation of signal from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                    |
| bqc_cv\_%                | Coefficient of variation of signal in % from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                |
| bqc_cv_robust\_%         | Robust coefficient of variation of signal in % from batch/pooled QC samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                         |
| tqc_mean                 | Mean signal from techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                    |
| tqc_median               | Median signal from techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                  |
| tqc_sd                   | Standard deviation of signal from techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                   |
| tqc_mad                  | Median absolute deviation of signal techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                 |
| tqc_cv\_%                | Coefficient of variation of signal in % from techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].        |
| tqc_cv_robust\_%         | Robust coefficient of variation in % of signal from techincal QC (or pooled lipid extracts) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018]. |
| spl_mean                 | Mean signal from experimental (unknown) samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                     |
| spl_median               | Median signal from experimental (unknown) defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                           |
| spl_sd                   | Standard deviation of signal from experimental (unknown) defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                            |
| spl_mad                  | Median absolute deviation of signal from experimental (unknown) defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                     |
| spl_cv\_%                | Coefficient of variation in % of signal from experimental (unknown) defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                 |
| spl_cv_robust\_%         | Robust coefficient of variation in % of signal from experimental (unknown) defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                          |
| pblk_median              | Median signal from processed blank samples defined by [Broadhurst et. al. 2018](https://doi.org/10.1007/s11306-018-1367-3) [@Broadhurst2018].                                                          |
...
```{r calculate QC sample statistics}
...
summary_data <- summary_data |> 
  ...
  tidyr::unnest(.data$sample_summary) |>
  dplyr::mutate(
...
```{r preview QC sample statistics}
#| code-fold: true
...
In this section, we create the palette for our dilution plots as well as our injection sequence plots. The [Okato-Ito palette](https://jfly.uni-koeln.de/color/index.html) [@OkabeIto2008] is used.

The relevant utility functions to do this are as follows.

```{r palatte utility functions, warning=FALSE, message=FALSE}
#| code-fold: true
...
```{r create qc sample type pal, warning=FALSE, message=FALSE}
...
```{r raincloud plot utility functions}
#| code-fold: true
...
```{r plot raincloud plot functions}
#| code-fold: true
...
```{r plotting raincloud plot, warning=FALSE}
...
```{r injection sequence plot utility functions}
#| code-fold: true
...
```{r plotting injection sequence plot, warning=FALSE}
...
injection_sequence_table <- raincloud_table |> 
  ...
  dplyr::select(-c("dilution_data","sample_data","no_rqc_data",
...
```{r preview injection sequence plot}
#| code-fold: true
...
The function [`trelliscopejs::cog`](https://hafen.github.io/trelliscopejs/reference/cog.html) is used to convert a column into cognostic object. The following meta information must be provided in order for the conversion to be successful.

Given a column name, we need to provide

* Description of the column name (meta information)
* The type of cognostics to used. For our example only "numeric" and "factor" are used. Other factor types can be found in the documentation of [`trelliscopejs::cog`](https://hafen.github.io/trelliscopejs/reference/cog.html)

Here are the relevant functions to create our meta information tibble.

```{r default cognostics functions}
#| code-fold: true
...
```{r before dilution curve cognostics}
...
```{r before injection sequence plot cognostics}
...
```{r injection sequence plot cognostics}
...
```{r after injection sequence plot cognostics 1}
attributes(injection_sequence_trellis$Transition_Name)
```

For the column labelled as a panel variable, we have

```{r after injection sequence plot cognostics 2}
attributes(injection_sequence_trellis$panel)
```

For the rest of the column converted to a common cognostics, we have

```{r after injection sequence plot cognostics 3}
...
```{r injection sequence plot trellis}
...
```{r dilution and injection sequence trellis print code, eval=FALSE}
trelliscope_name <- "Dilution Plot"
...
A `docs` folder will be created. It is currently placed in this [GitHub repository](https://github.com/JauntyJJS/Trelliscopejs_In_Quarto_Supplementary1). 

Below is just an embedded webpage of the trellis plot. It can be accessed via this [GitHub Page link](https://jauntyjjs.github.io/Trelliscopejs_In_Quarto_Supplementary1/).
