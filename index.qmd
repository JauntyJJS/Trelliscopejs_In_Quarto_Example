...
Here is how the data (first 50 rows) looks like
...
It is also possible to combined the two trellis plots together as they have the same conditioning cognostics variable. Simply set the `path` to the same folder name. In the example below, `path = "Results_Folder"` was used.
...
```{r dilution and injection sequence trellis run code, echo=FALSE}
trelliscope_name <- "Dilution Plot"
...
# Assign to a dummy variable to prevent it from printing to screen
dummy <- injection_sequence_trellis  |> 
  trelliscopejs::trelliscope(
    name = trelliscope_name,
    panel_col = "panel",
    path = "Results_Folder",
